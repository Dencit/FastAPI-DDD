...
from app.http.demo.request import SampleRequest
from app.http.demo.logic import SampleLogic
...
async def sample_update(request_input: SampleRequest.Update, id: int, auth: dict = Depends(open_auth)):
    """更新行"""
    print("auth::", auth)
    # 输入
    request_input = request_input.dict(exclude_none=True)
...
async def sample_delete(id: int, auth: dict = Depends(open_auth)):
    """删除行"""
    print("auth::", auth)
